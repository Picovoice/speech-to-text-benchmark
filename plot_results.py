...
def _plot_mem(save_folder: str, show: bool, dataset: Datasets = Datasets.TED_LIUM) -> None:
    fig, ax = plt.subplots(figsize=(6, 6))
    for engine_type, engine_value in MEMORY.items():
        max_mem_usage = engine_value[dataset] / 1000
    ...
    plot_path = os.path.join(save_folder, "mem_usage_comparison.png")
    os.makedirs(os.path.dirname(plot_path), exist_ok=True)
    plt.savefig(plot_path)
    print(f"Saved plot to `{plot_path}`")
...
def main() -> None:
    ...
    _plot_mem(save_folder=save_folder, show=args.show, dataset=Datasets.TED_LIUM)
